          ...
          for (const line of messageLines) {
            if (line.startsWith('data:')) {
              ...
              const lastExecutingThought = jsonData.output.thoughts
                ...
                .find(thought => {
                  try {
                    const parsedResponse = JSON.parse(thought.response) as NodeResponse;
              ...
              if (lastExecutingThought) {
                try {
                  const parsedResponse = JSON.parse(lastExecutingThought.response) as NodeResponse;
                  if (parsedResponse.nodeResult) {
                    const nodeResult = JSON.parse(parsedResponse.nodeResult) as NodeResult;
